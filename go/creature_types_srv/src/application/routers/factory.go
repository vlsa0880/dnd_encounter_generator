...
import (
	...
	ginrouter "github.com/vlsa0880/dnd_encounter_generator/go/creature_types_srv/src/application/routers/gin"
...
func New(ctx context.Context, settingsLoader settings.SettingsLoader, creatureTypesDB dbinterfaces.CreatureTypesRepository) ([]interfaces.Router, error) {
	...
	for _, routerName := range config.Router.Types {
		switch routerName {
		case "rest":
			ginRouter, err := ginrouter.New(settingsLoader, creatureTypesDB)
			if err != nil {
